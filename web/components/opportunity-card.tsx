...
export function OpportunityCard({ opportunity, demo = false }: { opportunity: Opportunity; demo?: boolean }) {
  return (
    <article className="opportunity-card reveal reveal--three">
      ...
      <div className="card__footer"><p className="card__evidence">{demo ? "CLAIM_TYPE: OBSERVATION / EVIDENCE READY" : `${opportunity.evidence_fact_ids.length} EVIDENCE FACTS / SCORE ${opportunity.score}`}</p><a className="card__action" href={demo ? "#radar" : `/opportunities/${opportunity.id}`}>DIAGNOSE →</a></div>
    </article>
  );
}