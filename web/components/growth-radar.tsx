...
export function GrowthRadar({ opportunities, isLive }: { opportunities: Opportunity[]; isLive: boolean }) {
  return (
    <section className="radar reveal reveal--three" id="radar" aria-labelledby="radar-title">
      ...
      <div className="flow" aria-label="Observe to learn process">{flow.map((step, index) => <div className={`flow__step ${index === 1 ? "flow__step--active" : ""}`} key={step}>{step}</div>)}</div>
      <div className="cards">
        {isLive && opportunities.map((opportunity) => <OpportunityCard key={opportunity.id} opportunity={opportunity} />)}
