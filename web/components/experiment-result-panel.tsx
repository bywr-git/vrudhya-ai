...
export function ExperimentResultPanel({ result, onLearn, learning }: { result: MeasurementResponse; onLearn: () => void; learning: boolean }) {
  return (
    <section className="experiment-result" aria-labelledby="measured-result-title">
      ...
      <div className="phase-panel__result-actions"><div className="phase-panel__disclaimer phase-panel__disclaimer--measured">MEASURED / BACKEND-CALCULATED RESULT</div><button type="button" className="phase-button" onClick={onLearn} disabled={learning}>{learning ? "LEARNING FROM RESULT" : "LEARN FROM RESULT →"}</button></div>
    </section>
  );
}