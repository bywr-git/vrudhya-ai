...
import { createExperimentDraft, diagnoseOpportunity, getDiagnosis, getHypotheses, getOpportunity, getStrategies } from "@/lib/api-client";
import type { Diagnosis, ExperimentDraft, Hypothesis, Opportunity, OpportunityDetail as OpportunityDetailData, Strategy } from "@/lib/api-types";
...
type WorkspaceState = "signal" | "diagnosing" | "diagnosed" | "simulating" | "simulation" | "permission" | "draft" | "error";
...
export function OpportunityDetail({ opportunity, onClose }: { opportunity: Opportunity; onClose: () => void }) {
  ...
  return (
    <section className="intelligence-workspace" aria-labelledby="workspace-title">
      ...
      <div className="workspace__flow"><span className="is-done">01 OBSERVE ✓</span><span className="is-done">02 DETECT ✓</span><span className={state === "signal" || state === "error" ? "is-active" : "is-done"}>{state === "diagnosing" ? "03 DIAGNOSE ●" : "03 DIAGNOSE ✓"}</span><span className={state === "diagnosed" ? "is-active" : ["simulating", "simulation", "permission", "draft"].includes(state) ? "is-done" : ""}>04 STRATEGIZE {state === "diagnosed" ? "●" : ["simulating", "simulation", "permission", "draft"].includes(state) ? "✓" : ""}</span><span className={["simulating", "simulation", "permission", "draft"].includes(state) ? "is-active" : ""}>05 EXPERIMENT {state === "simulating" || state === "simulation" || state === "permission" || state === "draft" ? "●" : ""}</span><span>06 MEASURE</span><span>07 LEARN</span></div>
      ...
      {state === "signal" || state === "error" ? <div className="workspace__action-row"><p>Review the persisted detector evidence before reasoning about possible causes.</p><button type="button" className="diagnose-button" onClick={diagnose}>DIAGNOSE SIGNAL →</button></div> : state === "diagnosing" ? <div className="analyzing-state" role="status"><span className="analyzing-state__mark">◌</span><div><strong>ANALYZING SIGNAL</strong><p>Loading persisted diagnosis and research context...</p></div></div> : state === "diagnosed" ? <div className="workspace__panels"><DiagnosisPanel diagnosis={diagnosis!} /><HypothesisPanel hypotheses={hypotheses} /><StrategyPanel strategies={strategies} onSimulate={simulate} simulatingStrategyId={simulatingStrategyId} /></div> : state === "simulating" ? <div className="analyzing-state" role="status"><span className="analyzing-state__mark">◌</span><div><strong>RUNNING SIMULATION</strong><p>Persisted assumptions are being evaluated by the backend.</p></div></div> : draft ? <div className="phase-stack"><SimulationPanel simulation={draft.simulation} />{state === "simulation" && <button type="button" className="phase-button phase-button--wide" onClick={() => setState("permission")}>CHECK PERMISSION →</button>}{state === "permission" && <PermissionPanel permission={draft.permission} onContinue={() => setState("draft")} />}{state === "draft" && <><PermissionPanel permission={draft.permission} onContinue={() => setState("draft")} />{draft.experiment && <ExperimentDraftPanel experiment={draft.experiment} />}</>}</div> : null}
    </section>
  );
}